...
@Component({
  selector: 'app-types-de-cuisine',
  imports: [FormsModule,
    NgbdSortableHeaderDirective,
    ReactiveFormsModule,
    NgbModule,
    AsyncPipe,],
  templateUrl: './types-de-cuisine.html',
  styleUrl: './types-de-cuisine.scss',
  providers: [TableService, DecimalPipe],
})
export class TypesDeCuisine {
 public service = inject(TableService);
  ...
  get_all_datas(){
    ...
    this.crudSaasService.getTypesCuisine(restaurant_id).subscribe({
      next: (res) => {
        this.service.setData(res);
        console.log("types_cuisine",this.types_cuisine)
      },
      error: (err) => {
  ...
  redirect_add(){
    this.router.navigate(['/types-de-cuisine/creer-type-cuisine']);
  }
  ...
  modifier_data(id:number){
    ...
    this.router.navigate(['/types-de-cuisine/modifier-type-cuisine', id]);
  }
     ...
     if (!this.canDelete()) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.crudSaasService.deleteTypeCuisine(id).subscribe({
              next: (res) => {
