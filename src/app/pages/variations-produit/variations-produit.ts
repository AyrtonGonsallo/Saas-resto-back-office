...
@Component({
  selector: 'app-variations-produit',
  imports:  [FormsModule,
    NgbdSortableHeaderDirective,
    ReactiveFormsModule,
    NgbModule,
    AsyncPipe,],
  templateUrl: './variations-produit.html',
  styleUrl: './variations-produit.scss',
  providers: [TableService, DecimalPipe],
})
export class VariationsProduit {
  ...
  variations_produit:any


  get_all_datas(){

    this.crudSaasService.getVariationsProduit().subscribe({
      next: (res) => {
        this.service.setData(res);
        console.log("variations_produit",this.variations_produit)
      },
      error: (err) => {
        this.notificationsService.error("Erreur lors de la récupération des variations_produit","Echec")
      }
    });
  }
  ...
  modifier_data(id:number){
    this.router.navigate(['/variations-produit/modifier-variation-produit', id]);
  }
  ...
  supprimer_data(id:number){
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.crudSaasService.deleteVariationProduit(id).subscribe({
              next: (res) => {
