...
@Component({
  selector: 'app-categories-produit',
   imports: [FormsModule,
    NgbdSortableHeaderDirective,
    ReactiveFormsModule,
    NgbModule,
    AsyncPipe,],
  templateUrl: './categories-produit.html',
  styleUrl: './categories-produit.scss',
...
export class CategoriesProduit {
  ...
  redirect_add(){
    this.router.navigate(['/categories-produit/creer-categorie-produit']);
  }
  ...
  modifier_data(id:number){
    ...
    this.router.navigate(['/categories-produit/modifier-categorie-produit', id]);
  }
   ...
   if (!this.canDelete()) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.crudSaasService.deleteCategorieProduit(id).subscribe({
              next: (res) => {
