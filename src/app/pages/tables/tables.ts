...
export class Tables {
  ...
  get_all_datas(){
    ...
    this.crudSaasService.getTables().subscribe({
      next: (res) => {
  ...
  redirect_add(){
    this.router.navigate(['/tables/creer-table']);
  }

  modifier_data(id:number){
    this.router.navigate(['/tables/modifier-table', id]);
  }
  ...
  supprimer_data(id:number){
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.crudSaasService.deleteTable(id).subscribe({
              next: (res) => {
