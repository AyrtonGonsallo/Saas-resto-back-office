...
@Component({
  ...
  templateUrl: './modifier-tag.html',
  styleUrl: './modifier-tag.scss',
})
export class ModifierTag {
  ...
  onSubmit() {
    ...
    this.crudSaasService.updateTag(this.data_id,this.formData.value).subscribe({
      next: (res) => {
        Swal.fire({
              position: 'bottom-end',
              icon: 'success',
  ...
  load_data(id:number){
    ...
    this.crudSaasService.getTagById(id).subscribe({
      next: (res) => {
        this.data=res
