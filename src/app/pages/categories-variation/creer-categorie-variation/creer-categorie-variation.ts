...
@Component({
  selector: 'app-creer-categorie-variation',
  imports: [ReactiveFormsModule,CommonModule, ReactiveFormsModule, NgSelectModule, NgbModule,AngularEditorModule,],
  templateUrl: './creer-categorie-variation.html',
  styleUrl: './creer-categorie-variation.scss',
})
export class CreerCategorieVariation {
  ...
  onSubmit() {
    ...
    console.log(this.formData.value);
    this.crudSaasService.ajouterCategorieVariation(this.formData.value).subscribe({
      next: (res) => {
        Swal.fire({
              position: 'bottom-end',
              icon: 'success',
