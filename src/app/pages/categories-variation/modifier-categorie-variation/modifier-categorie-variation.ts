...
@Component({
  selector: 'app-modifier-categorie-variation',
  imports: [ReactiveFormsModule,CommonModule, ReactiveFormsModule, NgSelectModule, NgbModule,AngularEditorModule,],
  templateUrl: './modifier-categorie-variation.html',
  styleUrl: './modifier-categorie-variation.scss',
})
export class ModifierCategorieVariation {
  ...
  verifier_roles_et_societes(user: any, currentRestaurant: any) {
    ...
    if (societeUser !== societeRestaurant) {
      this.notificationsService.error(
        "Vous ne pouvez pas modifier une catégorie de variation d'une autre société",
        "Echec"
      );
    ...
    if (prioriteRoleUser === 4) {
      ...
      if (!canAccess) {
        this.notificationsService.error(
          "Vous ne pouvez pas modifier cette catégorie de variation",
          "Echec"
        );
        this.router.navigate(['/dashboard/default']);
        return;
  ...
  onSubmit() {
    ...
    this.crudSaasService.updateCategorieVariation(this.data_id,this.formData.value).subscribe({
      next: (res) => {
        Swal.fire({
              position: 'bottom-end',
              icon: 'success',
        ...
        setTimeout(() => {
          this.router.navigate(['/categories-variation/liste-categories-variation']);
        }, 2000);
      },
      error: (err) => {
  ...
  load_data(id:number){
    ...
    this.crudSaasService.getCategorieVariationById(id).subscribe({
      next: (res) => {
        this.data=res
        ...
        this.formData = this.fb.group({
          ...
          ordre: [this.data.ordre, [Validators.pattern(/^[0-9]+$/), Validators.min(1), Validators.max(100)]],
