...
@Component({
  ...
  templateUrl: './ajouter-zone.html',
  styleUrl: './ajouter-zone.scss',
})
export class AjouterZone {
  ...
  onSubmit() {
    ...
    console.log(this.formData.value);
    this.crudSaasService.ajouterZoneRestaurant(this.formData.value).subscribe({
      next: (res) => {
        Swal.fire({
              position: 'bottom-end',
              icon: 'success',
