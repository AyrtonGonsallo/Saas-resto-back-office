...
@Component({
  selector: 'app-zones-restaurant',
  imports:  [FormsModule,
    NgbdSortableHeaderDirective,
    ReactiveFormsModule,
    NgbModule,
    AsyncPipe,],
  templateUrl: './zones-restaurant.html',
  styleUrl: './zones-restaurant.scss',
   providers: [TableService, DecimalPipe],
})
export class ZonesRestaurant {
  ...
  redirect_add(){
    this.router.navigate(['/zones-restaurant/creer-zone']);
  }

  modifier_data(id:number){
    this.router.navigate(['/zones-restaurant/modifier-zone', id]);
  }
  ...
  supprimer_data(id:number){
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            this.crudSaasService.deleteZoneRestaurant(id).subscribe({
              next: (res) => {
