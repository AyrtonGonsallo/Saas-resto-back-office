...
export class ModifierMenu {
   ...
   verifier_roles_et_societes(user: any, currentRestaurant: any) {
    ...
    if (societeUser !== societeRestaurant) {
      this.notificationsService.error(
        "Vous ne pouvez pas modifier un menu d'une autre société",
        "Echec"
      );
    ...
    if (prioriteRoleUser === 4) {
      ...
      if (!canAccess) {
        this.notificationsService.error(
          "Vous ne pouvez pas modifier ce menu",
          "Echec"
        );
        this.router.navigate(['/dashboard/default']);
        return;
  ...
  load_data(id:number){
    ...
    this.crudSaasService.getMenuById(id).subscribe({
      ...
      error: (err) => {
        if (err.status === 404) {
        this.notificationsService.error("Menu introuvable", "Echec");
      } else if (err.status === 400) {
        this.notificationsService.error("ID de menu invalide", "Echec");
      } else {
